...
func delayed_ready():
	if BackgroundScene.position_saver is Vector2:
		...
		if BackgroundScene.position_saver == 'left':
			position = $".."/Border_left.get_position() + Vector2($".."/Border_left.get_child(0).get_shape().get_size().x * 0.5 + 1, 0)
		elif BackgroundScene.position_saver == 'right':
			position = $".."/Border_right.get_position() + Vector2(($".."/Border_right.get_child(0).get_shape().get_size().x) * -0.5 - get_child(1).get_shape().get_size().x -1, 0)
		else:
